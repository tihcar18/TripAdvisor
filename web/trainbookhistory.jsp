<%-- 
    Document   : trainbookhistory
    Created on : 18-Apr-2015, 15:58:16
    Author     : win 8
--%>
...
<div class="clear"></div>
  ...
  <div class="container_12" >
      ...
      <center> <h2>Train Booking History</h2></center>            
     
          <table width="100%" border="1" class="mystyle" style="margin-left: -150px; margin-right: -50px;  line-height: 30px;">
        
            <tr><th>Username</th><th>Type</th><th>Departure From</th><th>Arrival To </th><th>Departure Date</th><th>Return Date</th><th>Class Preference</th><th>Train No.</th><th>No. of Tickets</th><th>Request No</th><th>Cost</th><th>Payment Date</th></tr>  
        <% p=c.prepareStatement("select * from trainbooking where uname=?");
         p.setString(1, b);
         rs=p.executeQuery();
         while(rs.next())
         {%>
   <tr ><td><%=rs.getString(1)%></td><td><%=rs.getString(2)%></td><td><%=rs.getString(3)%></td><td><%=rs.getString(4)%></td><td><%=rs.getString(5)%></td><td><%=rs.getString(6)%></td><td><%=rs.getString(7)%></td><td><%=rs.getString(8)%></td><td><%=rs.getString(10)%></td><td><%=rs.getString(11)%></td><td><%=rs.getString(12)%></td><td><%=rs.getString(13)%></td><td><a href="cancellation.jsp?x=Train">CANCEL</a></td></tr>  
 <% }
         %>        
          </table>
  </div>
