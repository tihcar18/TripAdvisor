<%-- 
    Document   : flightbookhistory
    Created on : 18-Apr-2015, 15:57:55
    Author     : win 8
--%>
...
<html>
    <head>
        ...
        <title>Flight Booking History</title>
      <meta charset="utf-8">
...
<div class="clear"></div>
  ...
  <div class="container_12" >
      ...
      <center>    <h2>Flight Booking History</h2></center>            
     
          <table width="100%" border="1" class="mystyle" style="margin-left: -150px; margin-right: -50px;  line-height: 30px;">
        
            <tr><th>Username</th><th>Type</th><th>Departure From</th><th>Arrival To</th><th>Departure Date</th><th>Return Date</th><th>Class Preference</th><th>Number of Tickets</th><th>Request No</th><th>Cost</th><th>Payment Date</th></tr>  
        <% p=c.prepareStatement("select * from flightbooking where uname=?");
         p.setString(1, b);
         rs=p.executeQuery();
         while(rs.next())
         {%>
   <tr ><td><%=rs.getString(1)%></td><td><%=rs.getString(2)%></td><td><%=rs.getString(3)%></td><td><%=rs.getString(4)%></td><td><%=rs.getString(5)%></td><td><%=rs.getString(6)%></td><td><%=rs.getString(7)%></td><td><%=rs.getString(9)%></td><td><%=rs.getString(10)%></td><td><%=rs.getString(11)%></td><td><%=rs.getString(12)%></td><td><a href="cancellation.jsp?x=Flight">CANCEL</a></td></tr>  
 <% }
         %>        
          </table> 
