<%-- 
    Document   : carbookhistory
    Created on : 18-Apr-2015, 15:57:43
    Author     : win 8
--%>
...
<div class="clear"></div>
  ...
  <div class="container_12" >
    ...
    <center>    <h2>Car Booking History</h2></center>            
     
 <table width="100%" border="1" class="mystyle" style="margin-left : -150px; margin-right : -50px;  line-height: 30px;">
        
            <tr><th>Username</th><th>Type</th><th>Pick-Up City</th><th>Drop-Off City</th><th>Pick-Up Date</th><th>Return Date</th><th>Pick-Up Time</th><th>Return Time</th><th>Car Class</th><th>Request No</th><th>Cost</th><th>Payment Date</th></tr>  
        <% p=c.prepareStatement("select * from carbooking where uname=?");
         p.setString(1, b);
         rs=p.executeQuery();
         while(rs.next())
         {%>
   <tr ><td><%=rs.getString(1)%></td><td><%=rs.getString(2)%></td><td><%=rs.getString(3)%></td><td><%=rs.getString(4)%></td><td><%=rs.getString(5)%></td><td><%=rs.getString(6)%></td><td><%=rs.getString(7)%></td><td><%=rs.getString(8)%></td><td><%=rs.getString(9)%></td><td><%=rs.getString(11)%></td><td><%=rs.getString(12)%></td><td><%=rs.getString(13)%></td><td><a href="cancellation.jsp?x=Car">CANCEL</a></td></tr>  
 <% }
         %>        
        </table> 
  </div>
