<%-- 
    Document   : packagebookhistory
    Created on : 18-Apr-2015, 15:58:36
    Author     : win 8
--%>
...
<div class="clear"></div>
  ...
  <div class="container_12" >
    ...
    <center>    <h2>Package Booking History</h2></center>            
     
         
          <table width="100%" border="1" class="mystyle" style="margin-left: -150px; margin-right: -50px;  line-height: 30px;">
        
            <tr><th>Username</th><th>Package ID</th><th>Package Type</th><th>Departure Date</th><th>Return Date</th><th>No. of Members</th><th>Request No</th><th>Cost</th><th>Payment Date</th></tr>  
        <% p=c.prepareStatement("select * from packagebooking where uname=?");
         p.setString(1, b);
         rs=p.executeQuery();
         while(rs.next())
         {%>
   <tr ><td><%=rs.getString(1)%></td><td><%=rs.getString(2)%></td><td><%=rs.getString(3)%></td><td><%=rs.getString(4)%></td><td><%=rs.getString(5)%></td><td><%=rs.getString(7)%></td><td><%=rs.getString(6)%></td><td><%=rs.getString(8)%></td><td><%=rs.getString(9)%></td><td><a href="cancellation.jsp?x=Package">CANCEL</a></td></tr>  
 <% }
         %>        
          </table> 
      </div>
